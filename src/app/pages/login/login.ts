...
@Component({
    ...
    <div class="auth-wrapper">
      ...
      <div class="auth-card">
        ...
        <div class="brand">
          ...
          <p class="tagline">// SECURE YOUR GEAR //</p>
        </div>

        <form [formGroup]="form" (ngSubmit)="onSubmit()">
          <div class="field">
            <label><span class="label-bracket">[</span> EMAIL <span class="label-bracket">]</span></label>
          ...
          <div class="field">
            <label><span class="label-bracket">[</span> PASSWORD <span class="label-bracket">]</span></label>
            <div class="input-wrap">
              <span class="input-icon">🔑</span>
              <input
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <circle cx="12" cy="12" r="3"/>
          ...
          <button type="submit" [disabled]="loading()" class="submit-btn">
            <span class="btn-text">
              @if (loading()) { AUTHENTICATING... } @else { ACCESS VAULT }
            </span>
        ...
        <p class="switch-link">
          NO ACCOUNT? <a routerLink="/register">CREATE VAULT</a>
        </p>
  ...
  styles: [`
    ...
    .label-bracket { color: rgba(255,255,255,0.3); }
...
export class LoginComponent {
  ...
  this.auth.login(this.form.value as any).subscribe({
    ...
    error: (err) => {
      console.log('FULL ERROR:', err);         // 👈 check console
      console.log('ERROR BODY:', err.error);   // 👈 check this
      this.errorMsg.set(
        err.error?.message || 
        err.message || 
